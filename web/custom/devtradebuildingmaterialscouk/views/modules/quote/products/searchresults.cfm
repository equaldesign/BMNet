...
<div class="widget widget-table">
  <table class="table table-bordered table-striped dataTable" id="productList">
	  ...
	  <tbody>
      <cfoutput>
	  	<cfloop array="#rc.results.items#" index="item" >
		  	<tr>
		  		<td>
            <cfif arrayLen(item.productImage) gte 1>
              <img class="thumbnail" src="https://www.buildingvine.com/api/productImage?nodeRef=#item.productImage[1].nodeRef#&size=50" >
            <cfelse>
              <img class="thumbnail" src="https://www.buildingvine.com/api/productImage?eancode=#item.eancode#&manufacturerproductcode=#URLEncodedFormat(paramValue("item.attributes.manufacturerproductcode",""))#&size=50&supplierproductcode=#URLEncodedFormat(paramValue("item.attributes.supplierproductcode",""))#&productName=#item.title#" >
            </cfif>
          </td>
					<td><h4><a class="modalProduct" data-toggle="modal" data-target="##productDetailModal" href="/quote/product/productDetail?nodeRef=#item.nodeRef#">#item.title#</a></h4>
					 <!--- <p>#custom2#</p>  --->
					</td>
					<td nowrap="nowrap">
						<input type="hidden" class="pcode" value="#item.nodeRef#" />
						<input type="hidden" class="utype" value="EA" />
						<input type="hidden" class="pname" value="#item.title#" />
...
<form class="form form-horizontal">
	<fieldset>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="input" class="input-small" value="1" id="manualQuantity">
