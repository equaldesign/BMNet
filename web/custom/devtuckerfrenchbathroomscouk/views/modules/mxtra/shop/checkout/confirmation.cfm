...
<div id="checkout">
  ...
  <div class="alert alert-success">
  	<h3>Confirmation</h3>
		<p>Please review and confirm your <cfif rc.delivered><cfif rc.order.quote>quotation<cfelse>order</cfif></a><cfelse>reservation</cfif> below</p>
  </div>
  <div class="row">
  ...
  <form action="/mxtra/shop/checkout/finish" method="post">
    ...
    <cfif rc.delivered>
		  <input type="submit" value="CONFIRM <cfif rc.order.quote>YOUR QUOTE<cfelse>&amp; COMMIT YOUR ORDER</cfif> &raquo;" class="btn btn-success">
		<cfelse>
      ...
      <table class="table table-striped table-bordered">
        ...
        <tr>
          ...
          <cfif rc.delivered><td>&pound;#DecimalFormat(VatPrice(item.deliverycost))#</td></cfif>
        </tr>
        </cfloop>

        <tr>
          <th class="bold" colspan="2" align="right">TOTAL<cfif rc.delivered> including Delivery</cfif></th>
          <th>&pound;#VatPrice(totalProductCost)#</th>
          <cfif rc.delivered><th>&pound;#VatPrice(totalDeliveryCost)#</th></cfif>
        </tr>
        </cfoutput>
      </table>
<cfoutput>
<div>
<div class="page-header">
  <h2 style="text-align:right">Total <cfif rc.delivered>including delivery<cfelse>due on collection</cfif>: &pound;#Trim(VatPrice(totalDeliveryCost+totalProductCost))#</h2>
</div>
<table width="100%">
<tr>
  <td colspan="2" valign="top">
  <cfif isUserInRole("trade") OR rc.order.quote eq false AND rc.delivered>	  
		  <table class="table table-striped table-bordered">
        <thead>
          <tr>
