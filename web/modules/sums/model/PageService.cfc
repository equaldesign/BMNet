...
<!--- Dependencies --->
  ...
  <cffunction name="proxy" returntype="any" output="false">
    ...
    <cfif JSONRequest>
      <cfhttp username="admin" password="bugg3rm33" port="8080" url="http://46.51.188.170/alfresco/service/#arguments.proxyurl#" method="#arguments.method#" result="requestResult">
    ...
    <cfif requestResult.statusCode neq "200 OK">
        <cfset returnStruct = {}>        
        <cfset returnStruct.error = requestResult>
    ...
    <cfif isDefined("returnStruct.page.url")>
      <cfhttp port="8080" url="http://10.50.14.61/alfresco#returnStruct.page.url#?ticket=#arguments.alf_ticket#" method="get" result="contentResult"></cfhttp>
  ...
  <cffunction name="publish" returntype="string">
    <cfargument name="nodeRef" required="true">
    <cfset returnData = proxy(
      proxyurl="/sums/page/checkin/a?nodeRef=#arguments.nodeRef#",
      formCollection={},
      method="get",
      JSONRequest=false,
    ...
    <cfset fileName = returnData.page.name>
    <!--- make sure there is a version in the cache --->
    <cfhttp url="http://#cgi.http_host#/html/#fileName#"></cfhttp>
    <!--- clear the proxy, but wait a second --->
    <cfthread action="sleep" duration="1000"></cfthread>
    <cfhttp url="http://#cgi.http_host#/purge/html/#fileName#"></cfhttp>
    <!--- create unsessioned cache version --->
    <cfhttp url="http://#cgi.http_host#/html/#fileName#"></cfhttp>
    <cfif fileName eq "homepage.html">
      <!--- clear the proxy --->
      <cfhttp url="http://#cgi.http_host#/purge/"></cfhttp>
      <!--- create unsessioned cache version --->
      <cfhttp url="http://#cgi.http_host#/"></cfhttp>
    </cfif>
    <cfreturn fileName>
  </cffunction>
