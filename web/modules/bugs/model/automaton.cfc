<cfcomponent>
  <cfproperty name="gmail" inject="coldbox:myPlugin:gmail" />
  <cffunction name="check" returntype="Any">
    <cfset var messages = "">
    <cfimap action="open" username="tickets@ebiz.co.uk" password="bugg3rm33" connection="cf.gmail" server="imap.gmail.com" secure="yes">
    <cfimap action="getall" attachmentpath="/fs/sites/ebiz/help.ebiz.co.uk/attach" generateUniqueFileNames="true" folder="INBOX" connection="cf.gmail" name="messages">
    <cfloop query="messages">
		  <cfset ticketWithin = "subject">
		  <cfset toEmail = ListGetAt(getEmails(to),1)>
      <cfimap action="delete" messageNumber="#messageNumber#" connection="cf.gmail">
      <cfset fromName = ReReplaceNoCase(from, "<[^>]*>", "", "ALL")>
      <cfset fromEmail = ListGetAt(getEmails(from),1)>
      ...
      <cfif ticketA.len[1] eq 0>
	     <cfset ticketA = ReFindNoCase("[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{16}",body,1,true)>
        ...
        <cfswitch expression="#ListGetAt(ListGetAt(toEmail,1,'@'),1,'.')#">
			    <cfcase value="eGroup">
					  ...
					  <cfelse>
					  	<cfset ticket = mid(body,ticketA.pos[1],ticketA.len[1])>
					  </cfif>
					  <cfset userObject = findUser(fromEmail,ticket)>
					  <cfset doeGroup(functionType,ticket,userObject,body)>
				  </cfcase>
          <cfcase value="buildersmerchant">
            ...
            <cfset userObject = findUser(fromEmail,ticket)>
            <cfset doBMNet(functionType,ticket,userObject,body)>
          </cfcase>
			    <cfdefaultcase>
              ...
              <cfset system = "BMNet">
		        ...
		        <cfif newTicket>
		          <cfquery name="alreadyexists" datasource="bugs">
		            select id from bug where description = <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#body#">
		          </cfquery>
		          <cfif alreadyexists.recordCount eq 0>
		            ...
		            <cfloop list="#ATTACHMENTFILES#" delimiters="#Chr(9)#" index="attach">
		            ...
		            <cfmail server="email-smtp.us-east-1.amazonaws.com" username="AKIAI3EY3YSOVJF37TRA" password="AsBr0dGv8iX6TJf0RPnihRzMhtw42VAeRWu/9rpuToD7" subject="Ticket: #ticket#" to="#from#" from="eBiz Support <#toEmail#>" bcc="tom.miller@ebiz.co.uk,james.colin@ebiz.co.uk">
...
DETAILS: #body#
              ...
              <cfset var commands = checkCommands(body,ticket)>
