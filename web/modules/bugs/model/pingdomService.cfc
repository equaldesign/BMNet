<cfcomponent displayname="pingdomAPI" output="false" hint="I am the pingdomAPI Class." cache="true">

  <cfproperty name="endpoint"     type="string" default="" />
  <cfproperty name="applicationKey"   type="string" default="se79wwtugbl3sm968m5fu5ikj8s6a09h" />
  ...
  <cfscript>
    variables.instance.emailAddress   = "tom.miller@ebiz.co.uk";
    variables.instance.password     = "bugg3rm33";
    variables.instance.applicationKey   = "se79wwtugbl3sm968m5fu5ikj8s6a09h";
    variables.instance.parse      = true;
  </cfscript>
  ...
  <cffunction name="getHoursOfDayAverage" access="public" output="false" hint="Get the average response time and uptime for a list of intervals. Useful for generating graphs.">
    <cfargument name="checkid"      required="true"   type="Numeric"  hint="Identifier of target check." />
    <cfargument name="from"       required="false"  type="Numeric"  hint="Start time of period. Format is UNIX timestamp." />
    <cfargument name="to"         required="false"  type="Numeric"  hint="End time of period. Format is UNIX timestamp." />
    <cfargument name="probes"       required="false"  type="String"   hint="Filter to only use results from a list of probes. Format is a comma separated list of probe identifiers. Can not be used if includeuptime is set to true. Also note that this can cause intervals to be omitted, since there may be no results from the desired probes in them." />
    <cfargument name="uselocaltime"  required="false"  type="Boolean"  hint="If true, use the user's local time zone for results (from and to parameters should still be specified in UTC). If false, use UTC for results." />

    <cfset var strURL   = '' />
    <cfset var returnData = '' />
    <cfset strURL   = 'summary.hoursofday/#arguments.checkid#?' & buildParamString(arguments) />
    <cfset returnData = makeCall(
      url   = strURL,
      method  = 'GET'
            ) />
      ...
      <cfset var cfhttp = '' />
        ...
        <cfhttp url="#strURL#"
          ...
          <cfhttpparam type="header"
                name="App-Key"
                value="se79wwtugbl3sm968m5fu5ikj8s6a09h" />
