<cfcomponent name="poi" hint="Creates a upload" extends="coldbox.system.plugin" cache="false" cachetimeout="50">

  <!--- CONSTRUCTOR --->
  <cffunction name="init" access="public" output="false" returntype="poi">
        <cfargument name="controller" type="any" required="true">
        <cfset super.Init(arguments.controller) />
        <cfset setpluginName("Apache POI Plugin")>
        <cfset setpluginVersion("1.0")>
        <cfset setpluginDescription("Pugin for the Apache POI.")>
  ...
  <cffunction name="write" access="public" output="false" returntype="void"
    ...
    <cfelse>
      <cfif getWorkbook().getNumberOfSheets() eq 0>
        <cfif getWorkbook().getSheetAt(JavaCast("int", getWorkbook().getNumberOfSheets() - 1)) neq 0>
    ...
    <cfif StructKeyExists(arguments, "query")>
      ...
      <cfloop query="arguments.query">
        ...
        <cfloop index="i" from="1" to="#ListLen(queryColumnList)#">
          <cfset v = arguments.query[ListGetAt(queryColumnList, i)][arguments.query.CurrentRow]>
          <cfif isNumeric(v)>
            <cfset nFormat = "double">
          <cfelse>
            <cfset nFormat = "string">
          </cfif>
          <cfset cell = row.createCell(JavaCast("int", i - 1)) />
          <cfset cell.setCellValue(JavaCast("#nFormat#", v)) />
        </cfloop>
      </cfloop>
  ...
  <cffunction name="update" access="public" output="false" returntype="void"
    ...
    <cfset arguments.workbookToUpdate = getWorkbook() />
    <cfset arguments.overwrite = true />

    <cfset write(argumentcollection = arguments) />
  </cffunction>

  <cffunction name="getSheet" access="public" output="false" returntype="query">
    ...
    <cfset row = getActiveSheet().getRow(0) />
     ...
     <cfset i = 1 />
     <cfloop condition="#cellIterator.hasNext()#">
       <cfset queryColumnName = ListToArray(getCellValue(row.getRowNum() + 1, cellIterator.next().getColumnIndex() + 1)," ") />
       <cfset queryColumnNames = queryColumnNames & queryColumnName[1] & "," />
       <cfset i = i + 1 />
     </cfloop>
     <cfset queryColumnNames = Left(queryColumnNames, Len(queryColumnNames) - 1) />
     <cfset query = QueryNew(queryColumnNames) />
  ...
  <cffunction name="getHeader" access="public" output="false" returntype="any"
      hint="Gets the header values on the sheet">
        <cfset var header = structNew()>
        <cfset header.left = getActiveSheet().getHeader().getLeft()>
        <cfset header.center = getActiveSheet().getHeader().getCenter()>
      <cfreturn header />
  </cffunction>
  ...
  <cffunction name="addRow" access="public" output="false" returntype="void"
    ...
    <cfargument name="data" type="string" required="true" hint="Comma-delimited list of data" />
    ...
    <cfloop list="#arguments.data#" index="cellValue">
  ...
  <cffunction name="formatCell" access="public" output="false" returntype="void"
    ...
    <cfif isDefined('cell') AND NOT isSimpleValue(cell)>
  ...
  <cffunction name="setAutoColumnWidth" access="public" output="false" returntype="void"
      hint="Sets the width of a column">
    <cfargument name="column" type="numeric" required="true" />
    <cfset getActiveSheet().autoSizeColumn(JavaCast("int", arguments.column - 1)) />
  </cffunction>
  ...
  <cffunction name="protectIt" access="public" output="false" returntype="any">
    <cfreturn getWorkbook().getSheetAt(JavaCast("int", getWorkbook().getActiveSheetIndex())).protectSheet("test") />
  </cffunction>
  ...
  <cffunction name="buildCellStyle" access="private" output="false" returntype="any"
    ...
    <cfloop collection="#arguments.format#" item="setting">
      <cfswitch expression="#setting#">
        ...
        <cfcase value="bgcolor">
          <cfset cellStyle.setFillBackgroundColor(getColorIndex(StructFind(arguments.format, setting))) />
        </cfcase>
